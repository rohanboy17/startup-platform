    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={() => runUserAction("SOFT_DELETE")} disabled={loading !== null}>Soft Delete</Button>
          <Button variant="outline" onClick={() => runUserAction("RESTORE")} disabled={loading !== null}>Restore</Button>
          <Button variant="outline" onClick={() => runUserAction("ANONYMIZE")} disabled={loading !== null}>Anonymize</Button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-5 space-y-3">
        ...
        <Button onClick={addEvidence} disabled={loading !== null}>{loading === "evidence" ? "Saving..." : "Add Evidence"}</Button>

        <div className="space-y-2">
          {initialEvidence.map((item) => (
            <div key={item.id} className="rounded-md border border-white/10 bg-black/20 p-3 text-sm">
              <p className="font-medium">{item.entityType} | {item.entityId}</p>
              ...
              {item.fileUrl ? <p className="text-xs text-sky-300">{item.fileUrl}</p> : null}
            </div>
          ))}
        </div>
      </div>
