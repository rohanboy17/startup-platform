...
export async function PATCH(
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const next = await tx.jobApplication.update({
      where: { id: applicationId },
      data: {
        adminStatus: action === "APPROVE" ? "ADMIN_APPROVED" : "ADMIN_REJECTED",
        adminReason: action === "REJECT" ? reviewReason : null,
    ...
    await tx.activityLog.create({
      data: {
        ...
        action: action === "APPROVE" ? "ADMIN_APPROVED_JOB_APPLICATION" : "ADMIN_REJECTED_JOB_APPLICATION",
  ...
  await Promise.all([
    sendInAppNotification({
      ...
      title: action === "APPROVE" ? "Job application cleared for business review" : "Job application rejected by admin",
      message:
        action === "APPROVE"
      ...
      templateKey: action === "APPROVE" ? "job.application_admin_approved" : "job.application_admin_rejected",
      payload: {
        applicationId,
  ...
  return NextResponse.json({
    message: action === "APPROVE" ? "Application approved for business review" : "Application rejected",
    application: updated,
  });
}
