...
export async function GET(req: Request) {
  ...
  const where: Prisma.BusinessFundingWhereInput = {
  ...
  const rows = await prisma.businessFunding.findMany({
    where,
    select: {
      amount: true,
      referenceId: true,
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="manual-business-funding.csv"',
    },
  });
}
